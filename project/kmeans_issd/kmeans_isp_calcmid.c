...
int kmeans(int T){
	...
	FILE* fpdata=fopen("kdata", "rb");
	FILE* fpclust=fopen("kclust", "rb");
	FILE* output;

	readkmeansb(kdata, fpdata, N);
	readkmeansb(klist, fpclust, K);
	calcclustmid(kdata, klist);
	fclose(fpclust);
	output=fopen("kclust", "wb");
	savekmeansb(klist, fpclust, K);
